...
if(isset($_POST['updatefood-submit'])) {
	...
	if (isEmpty($name) || isEmpty($carbs) || isEmpty($fiber) || isEmpty($sugar) || isEmpty($protein) || isEmpty($fat) || isEmpty($calorie)) { // ÜRES MEZŐK
		header("Location: ../../food.php?err=emptyfields&id=". $id ."&name=".$name."&carbs=".$carbs."&fiber=".$fiber."&sugar=".$sugar."&protein=".$protein."&fat=".$fat."&calorie=".$calorie);
        exit();
        
	} else {
		$sql = "UPDATE `foods` SET `name` = ?, `carbs` = ?, `fiber` = ?, `sugar` = ?, `protein` = ?, `fat` = ?, `calorie` = ? WHERE id = ?;"; 
		...
		if (!mysqli_stmt_prepare($stmt, $sql)) { // statement prepare kezelés
			...
			mysqli_stmt_bind_param($stmt, "sdddddis", $name, $carbs, $fiber, $sugar, $protein, $fat, $calorie, $id);  // parameteradás
